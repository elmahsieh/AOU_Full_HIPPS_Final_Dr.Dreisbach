...
def main(HIP_eps,PPS_eps,HIP_concept_file,PPS_concept_file):
    ...
    person_ids = singles['person_id'].unique()

    
    # Person-level merge
    # Join pregnancy and smoking status
    #
    preg_status = utilities.pull_pregnancy_status(person_ids)
    smoking_status = utilities.pull_smoking_status(person_ids)
    
    singles = utilities.flag_currently_pregnant(singles, preg_status)
    singles = utilities.merge_smoking_status(singles, smoking_status)
    
    singles = utilities.add_parity_column(singles)
    
    # Episode-level merge
    # Join parity, age_at_delivery, and delivery_method
    #
    demographics_df = utilities.pull_demographics(person_ids)
    
    singles = utilities.add_age_at_merged_end(singles, demographics_df)
    
    delivery_method_df = utilities.pull_delivery_method(person_ids)

    singles = utilities.merge_delivery_method(singles, delivery_method_df)
